...
extension SettingsController {
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        label.textColor = K.Color.black
        label.text = objectArray[section].sectionName
        label.textAlignment = .left
        view.addSubview(label)
        view.backgroundColor = K.Color.veryLightGray
        return view
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 0 {
            ...
            case 0:
                ...
                showLoader(true)
                AuthService.singoutUser { error in
                    self.showLoader(false)
                    if let error = error {
                        print("DEBUG: Failed to sing out user \(error.localizedDescription)")
                        return
                    }
